...
export default function Programmers() {
  ...
  const [programmerID, setProgrammer] = useState(listOfAwesome);
  const [featuredProgrammer, setFeatured] = useState(null)
  const getNameOfFeatured = () => {
    ...
    const programmerF = featuredProgrammer - 1;
    const id = programmerID[programmerF].name;
    return id;
  ...
  const style = {
    ...
    color: featuredProgrammer != null ? 'gold' : 'royalblue',
  };
